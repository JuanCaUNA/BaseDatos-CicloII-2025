    ...
    CURSOR C_LINES IS
        ...
        FROM TABLE(
            SYS.ODCIVARCHAR2LIST(
                ...
                'ACS_PERSONA,1,1,1,1',
                'ACS_USUARIO,1,1,1,1',
                'ACS_DOCUMENTO_USUARIO,1,1,1,0',
                'ACS_TIPO_DOCUMENTO,1,0,0,0',
                'ACS_CUENTAS_BANCARIA,1,1,1,0',
                'ACS_CENTROS_MEDICO,1,1,1,0',
                'ACS_ESCALAS_MENSUAL,1,1,1,1',
                'ACS_PLANILLA,1,1,1,0'
            )
        );
BEGIN
    FOR R_LINE IN C_LINES LOOP
    ...
    CURSOR C_LINES IS
        ...
        FROM TABLE(
            SYS.ODCIVARCHAR2LIST(
                ...
                'MEDICO GENERAL,Perfil orientado a la atención médica general',
                'ADMINISTRATIVO GENERAL,Perfil para operaciones administrativas transversales',
...
BEGIN
    ...
    SELECT APF_ID INTO V_APF_ID FROM ACS_PERFIL WHERE APF_NOMBRE = 'ADMINISTRADOR';
    FOR R IN (SELECT APR_ID FROM ACS_PERMISO) LOOP
        INSERT INTO ACS_PERFIL_PERMISO (APR_ID, APF_ID) VALUES (R.APR_ID, V_APF_ID);
    END LOOP;
    
    -- MEDICO GENERAL: pantallas clínicas principales
    SELECT APF_ID INTO V_APF_ID FROM ACS_PERFIL WHERE APF_NOMBRE = 'MEDICO GENERAL';
    FOR R IN (
        SELECT APR_ID FROM ACS_PERMISO
        WHERE APR_PANTALLA IN (
            'ACS_PERSONA',
            'ACS_USUARIO',
            'ACS_DOCUMENTO_USUARIO',
            'ACS_CENTROS_MEDICO',
            'ACS_ESCALAS_MENSUAL'
        )
    ) LOOP
    ...
    -- ADMINISTRATIVO GENERAL: gestión operativa y financiera
    SELECT APF_ID INTO V_APF_ID FROM ACS_PERFIL WHERE APF_NOMBRE = 'ADMINISTRATIVO GENERAL';
    FOR R IN (
        SELECT APR_ID FROM ACS_PERMISO
        WHERE APR_PANTALLA IN (
            'ACS_PERSONA',
...
DECLARE
    ...
    V_APF_ID NUMBER;
BEGIN
    -- Usuario 118690700 (Juan Camacho) - ADMINISTRADOR y MEDICO GENERAL
    SELECT U.AUS_ID INTO V_AUS_ID
    FROM ACS_USUARIO U
    INNER JOIN ACS_PERSONA P ON U.APE_ID = P.APE_ID
    ...
    -- Usuario CED345678 (Carlos Ramirez) - MEDICO GENERAL
    SELECT U.AUS_ID INTO V_AUS_ID
    FROM ACS_USUARIO U
    INNER JOIN ACS_PERSONA P ON U.APE_ID = P.APE_ID
    ...
    -- Usuario CED678901 (Laura Hernandez) - ADMINISTRATIVO GENERAL
    SELECT U.AUS_ID INTO V_AUS_ID
    FROM ACS_USUARIO U
    INNER JOIN ACS_PERSONA P ON U.APE_ID = P.APE_ID
    ...
    -- Usuario CED890123 (Sofia Jimenez) - ADMINISTRATIVO GENERAL
    SELECT U.AUS_ID INTO V_AUS_ID
    FROM ACS_USUARIO U
    INNER JOIN ACS_PERSONA P ON U.APE_ID = P.APE_ID
