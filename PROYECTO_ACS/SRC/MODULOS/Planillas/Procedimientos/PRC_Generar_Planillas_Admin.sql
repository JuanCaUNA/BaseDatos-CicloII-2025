CREATE OR REPLACE PROCEDURE PRC_Generar_Planillas_Admin(
  ...
  v_tipo_id      NUMBER;
  v_salario      NUMBER := 600000; -- salario base ejemplo
  v_deduccion    NUMBER := 0.08;   -- 8% CCSS, por ejemplo
BEGIN
  -- 1️⃣ Obtener tipo de planilla de administrativos
  SELECT ATP_ID INTO v_tipo_id
  FROM ACS_TIPO_PLANILLA
  WHERE UPPER(ATP_APLICA_A) = 'ADMINISTRATIVOS' AND ATP_ESTADO = 'ACTIVO';

  -- 2️⃣ Crear encabezado
  INSERT INTO ACS_PLANILLA (
    APL_ID, APL_ANIO, APL_MES, APL_ESTADO, APL_FEC_GEN,
    APL_TOT_BRUTO, APL_TOT_DED, APL_TOT_NETO,
    APL_FECHA_CREACION, APL_FECHA_ACTUALIZACION, ATP_ID
  ) VALUES (
    SEQ_ACS_PLANILLA.NEXTVAL, p_anio, p_mes, 'GENERADA', SYSTIMESTAMP,
  ...
  FOR reg IN (
    ...
    INSERT INTO ACS_DETALLE_PLANILLA (
      ADP_ID, ADP_TIPO_PERSONA, ADP_SALARIO_BASE, ADP_BRUTO, ADP_DED,
      APD_NETO, ADP_EMAIL_ENV, ADP_FECHA_CREACION, ADP_FECHA_ACTUALIZACION,
      APL_ID, AUS_ID
    ) VALUES (
      SEQ_ACS_DETALLE_PLANILLA.NEXTVAL, 'ADMINISTRATIVO', v_salario,
      v_salario, v_salario * v_deduccion, v_salario - (v_salario * v_deduccion),
      0, SYSTIMESTAMP, SYSTIMESTAMP, v_planilla_id, reg.AUS_ID
    );
  END LOOP;
  ...
  COMMIT;
  DBMS_OUTPUT.PUT_LINE('✅ Planilla de administrativos generada correctamente.');
EXCEPTION
  WHEN OTHERS THEN
    ROLLBACK;
    RAISE_APPLICATION_ERROR(-20002, 'Error al generar planilla de administrativos: ' || SQLERRM);
END;
/
